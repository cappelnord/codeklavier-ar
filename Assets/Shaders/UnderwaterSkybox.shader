 ...
 Pass {
    ...
    fixed4 frag (vertexOutput input) : COLOR
    {
      // main color gradient
       ...
       float moldA = tex2D(_MoldMap, float2(yn + x * sin(t * 4.0 * z * 2.0) * 0.1, z)) * 0.1;
       float moldB = (1.0 - tex2D(_MoldMap, float2(xn + x * sin(t * 1.0 * z * 3.0) * 0.1, z))) * 0.05;

       float moldC = tex2D(_MoldMap, float2(yn + z * sin(t * 3.14 * y * 4.0) * 0.001, z * 0.1 + sin(t * 0.02))) * 0.1 * smoothstep(0.45, 0.55, yn);
